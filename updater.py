...
def update_once(args, save_method, pre_data={}):
    ...
    for uid, item in tqdm(mapping.items()):
        ...
        if item['gamer'] is not None or  allCount >= 4 or item['isHentai']:  
            # if item['mal'] > 50:
            #     continue
            mal_res = myanimelist.get_anime_detail(item['mal'], True, MAL_DIR,False)
            if mal_res is None or mal_res['type'] not in allow_types:
                time.sleep(args.delay)
                continue
            else:
                print('redditanimelist start...')
                # redditanimelist_res = redditanimelist.get_anime_detail(item['mal'],True,REDDITANIMELIST_DIR,False)
                redditanimelist_res = None
            if item['livechart'] is not None:
                    print('livechart start...')
                    # livechart_res = None
                    livechart_res = livechart.get_anime_detail(item['livechart'], True, LIVECHART_DIR,False)
            else:   
                    livechart_res = None  
            if item['ann'] is not None:
                    print('anime_news_network start...')
                    ann_res = anime_news_network.get_anime_detail(item['ann'], True, ANN_DIR,False)
            else:
                    ann_res = None
            if item['bgm'] is not None:
                    print('bangumi start...')
                    bgm_res = bangumi.get_anime_detail(item['bgm'], True, BGM_DIR,False)
            else:
                    bgm_res = None
            if item['anilist'] is not None:
                    print('anilist start...')
                    anl_res = anilist.get_anime_detail(item['anilist'], True, ANL_DIR,False)
            else:
                    anl_res = None
            if item['anikore'] is not None:
                    print('anikore start...')
                    akr_res = anikore.get_anime_detail(item['anikore'], True, AKR_DIR,False)
            else:
                    akr_res = None    
            if item['gamer'] is not None:
                    print('gamer start...')
                    gamer_res = gamer.get_anime_detail(item['gamer'], True, GAMER_DIR,False)
            else:
                    gamer_res = None   
            if item['animePlanetCom'] is not None:
                    print('animePlanetCom start...')
                    animePlanetCom_res = anime_planet.get_anime_detail(item['animePlanetCom'], True, ANIMEPLANECOM_DIR,False)
            else:
                    animePlanetCom_res = None           
            if item['anisearch'] is not None:
                    print('anisearch start...')
                    anisearch_res = anisearch.get_anime_detail(item['anisearch'], True, ANISEARCH_DIR,False)
            else:
                    anisearch_res = None      
            if item['kitsu'] is not None:
                    print('kitsu start...')
                    kitsu_res = kitsu.get_anime_detail(item['kitsu'], True, KITSU_DIR,False)
            else:
                    kitsu_res = None    
            
            if item['notifyMoe'] is not None:
                    print('notifyMoe start...')
                    notifyMoe_res = notify_moe.get_anime_detail(item['notifyMoe'], True, NOTIFYMOE_DIR,False)
                    # notifyMoe_res = None
            else:
                    notifyMoe_res = None     
            if notifyMoe_res is not None and 'traktId' in notifyMoe_res and notifyMoe_res['traktId'] not in error_trakt:
                    print('trakt start...')
                    # trakt_res = None
                    trakt_res = trakt.get_anime_detail(notifyMoe_res['traktId'], True, TRAKT_DIR,False)
            ...
            if item['sakuhindb'] is not None:
                    ...
                    sakuhindb_res = sakuhindb.get_anime_detail(item['sakuhindb'], True, SAKUHINDB_DIR,False)
            else:
                    sakuhindb_res = None    
            if 'annict' in item: #必要
                    if item['annict'] is not None:
                            print('annict start...')
                            annict_res = annict.get_anime_detail(item['annict'], True, ANNICT_DIR,False)
                            # annict_res = None  
                    else:
                            annict_res = None    
            else:
            ...
            if item['anidb'] is not None:
                    anidb_res = None
                    print('anidb start...')
                    # anidb_res = anidb.get_anime_detail2(item['anidb'], True, ANIDB_DIR,False)
            else:
                    anidb_res = None    
            ...
            if livechart_res is not None :
                if livechart_res['online']:
                    for onlineId, item in livechart_res['online'].items():
                        ...
                        and onlineId.find('i-fun') == -1 and onlineId.find('Twitter') == -1 and onlineId.find('Instagram') == -1 and onlineId.find('Plex') == -1 and onlineId.find('Steam') == -1\
                        and onlineId.find('Tencent Video') == -1 and onlineId.find('AnimeLog') == -1 :
                            onlineIdName = ''
